...
export const SuperAdmDashboard: React.FC = () => {
  ...
  useEffect(() => {
    // MODO ESTÁTICO: Carrega direto do store
    setTimeout(() => {
      setData({
        kpis: {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-slate-400 font-medium italic animate-pulse">Sincronizando Base de Dados Local...</p>
      </div>
    );
  }
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden border border-indigo-500/20 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 font-medium italic">Gestão centralizada de infraestrutura (Modo Local/Static).</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 text-sm">Controle de acessos e monitoramento.</p>
          </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredUsers.length > 0 ? filteredUsers.map(u => (
                ...
                <tr>
                  <td colSpan={5} className="p-20 text-center text-slate-400 italic">Nenhum usuário encontrado na base.</td>
                </tr>
              )}
            </tbody>
          </table>
